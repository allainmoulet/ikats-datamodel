...
/**
 * The TableInfo is mapping the functional IKATS type 'table', as a JSON resource. TableInfo class can be used as a JSON
 * resource in the REST services, to be developed in TableResource. <br/>
 * The TableInfo is composed of one TableDesc section, one TableHeaders section and finally one TableContent section.
 * Each section is detailed below. <br/>
 * Note the difference with Table: the business resource Table is a wrapper of TableInfo, managed by TableManager, and
 * providing end-user services in java world.
 */
public class TableInfo {
    ...
    static public class TableContent {
        /**
         * The data directly contained by this table: cells structured as a list of rows. A row is a list of cell. A
         * cell is an Object, whose effective type is not fixed.
         * <p>
         * Optional: may be missing if links are defined.
         */
        public List<List<Object>> cells;
        /**
         * The default_links just provides mutual properties, shared by all this.links, in order to sum the link
         * information, optimizing the json serialized from Table class. <br/>
         ...
         * <p>
         * <br/>
         * Optional: if there is no links, or no mutual information shared by DataLink objects. <br/>
         * Note: if one DataLink redefines default_links.type
         */
        public DataLink default_links;

        /**
         * The data linked by this table: links structured as a list of rows. A row is a list of links. A link is an
         * DataLink.
         * <p>
         ...
         * <p>
         * Restriction: it is assumed that the defined objects under content.cells are immutable: String, Number,
         * BigDecimal, BigInteger, Boolean (...), otherwise you may have side-effects concerning the copy of
         * content.cells.
         *
         * @param content
         *            content to copy
        ...
        /**
         * Gets from this content the row as list of TableElement, at index, with optional links. Use this method if
         * TableContent is managing links, otherwise it will throw exception.
         *
         * @param index
         *            row index to get
         * @param requiresLinksOrDie
         *            true will activate additional checks on links consistency: this.links must be defined, and have
         *            same size than this.cells
         * @return the row
         * @throws IkatsException
         *             when this.cells is null or this.links is null or when cells.size() != links.size()
         * @throws IndexOutOfBoundsException
         *             when index is out of bound of this.cells
         */
        @JsonIgnore
        public List<TableElement> getRowDataWithLink(int index, boolean requiresLinksOrDie)
                throws IkatsException, IndexOutOfBoundsException {
            ...
            if (isHandlingLinks() && links.size() > index) {
            ...
            for (List<Object> row : cells) {
                if (row == null)
                    throw new IndexOutOfBoundsException(
                            "Failed: getSimpleDataColumn at row=" + posRow + " : row is null");

                if (index + 1 > row.size())
                    throw new IndexOutOfBoundsException("Failed: getSimpleDataColumn at row=" + posRow
                            + " : row size < (index + 1) with index=" + index);
        ...
        /**
         * Gets from this content the column as list of TableElement, at index, with optional links. Use this method if
         * TableContent is managing links, otherwise it will throw exception.
         *
         * @param index
         * @param requiresLinksOrdie
         ...
         * @throws IkatsException
         *             inconsistency error. For instance undefined cells, unexpected undefined links.
         * @throws IndexOutOfBoundsException
         *             when index is out of bound of at least one of the rows
         */
        @JsonIgnore
        public List<TableElement> getColumnDataWithLink(int index, boolean requiresLinksOrdie)
                throws IkatsException, IndexOutOfBoundsException {
            int posRow = 0;
            try {
                ...
                Iterator<List<DataLink>> iterLinks = isHandlingLinks() ? links.iterator() : null;
                ...
                throw new IkatsException(
                        "Failed: getColumn at row=" + posRow + " : list size < (index + 1) with index=" + index, iob);
            }
        }
         ...
         *
         * Note: if this.cells is not null, also allocates undefined links to null in order to have same dimensions on
         * this.cells and this.links.
         *
         * @param defaultProperties
         *            configuration of the links is providing default values in order to reduce the volume of JSON.
         */
         ...
         * @return this : permits to chain the modifiers: this.addRow(...).addRow(...)
         * @throws IkatsException
         *             inconsistency error. Example: trying to add a link in TableContent which is not managing links.
         */
        public TableContent addRow(List<TableElement> elements) throws IkatsException {
            int posCol = 0;
            int posRow = cells.size();
            List<Object> rowData = new ArrayList<>();
            boolean manageLinks = isHandlingLinks();
            ...
            for (TableElement elem : elements) {
                ...
                if (manageLinks) {
                    ...
                    throw new IkatsException("Failed to add new row at " + posRow + " column=" + posCol
                            + ": links not managed but got " + elem.link);
                }
                posCol++;
            }
         ...
         *
         * @param beforeIndex
         *            row specified with elements are inserted at position beforeIndex, an the rows previously at
         *            position in range from beforeIndex to the last index, are shifted to the next index.
         * @param elements
         *            elements specifying the inserted row. Each TableElement defines for its position data and link
         *            parts (null accepted when not data or link is undefined).
        ...
        public TableContent insertRow(int beforeIndex, List<TableElement> elements) throws IkatsException {
            ...
            try {
                List<Object> rowData = new ArrayList<>();
                List<DataLink> rowLinks = isHandlingLinks() ? new ArrayList<>() : null;
        ...
        /**
         * Adds a new column as the last one of the table.
         *
         * @param elements
         *            specifying the added column. Each TableElement defines for its position data and link parts (null
         *            accepted when not data or link is undefined).
         * @return this TableContent. Such return enables chained calls, for example:
         *         content.addColumn(c1).addColumn(c2).
         * @throws IkatsException
         */
        public TableContent addColumn(List<TableElement> elements) throws IkatsException {
            ...
            for (TableElement tableElement : elements) {
                ...
                if (manageLinks) {
                    ...
                    throw new IkatsException("Failed to add new row at " + posRow + " column=" + posCol
                            + ": links not managed but got " + tableElement.link);
                }
                posRow++;
            }
        ...
        private boolean isHandlingLinks() {
            return links != null;
        }

        /**
         * Inserts the specified column at the specified position in this list (optional operation). Shifts the columns
         * currently at that position (if any) and any subsequent columns to the right (adds one to their indices).
         * <p>
         * This operation updates as well this.cells and -if not null- this.links
         *
         * @param beforeIndex
        ...
        public TableContent insertColumn(int beforeIndex, List<TableElement> elements) throws IkatsException {
            ...
            try {
                ...
                throw new IkatsException(
                        "Failed to insert column at index=" + beforeIndex + " near row index=" + posRow, e);
            }
         ...
         * @param defaultLink
         *            when not null: the DataLink defining the default properties applicable to content links.
         * @return the created TableContent
         * @throws IkatsException
         *             inconsistency error during initialization.
         */
        static TableContent initContent(List<List<Object>> cellData, List<List<DataLink>> links, DataLink defaultLink)
                throws IkatsException {

            TableContent content = new TableContent();
            if (links == null && defaultLink != null)
                throw new IkatsException(
                        "Inconsistency: content cannot have defined default link if links are not managed");
    ...
    static public class DataLink implements Serializable {
        ...
        /**
         * The parameter value defining the link to the data. For example: the ID value of linked data (processdata, TS,
         * ...). Note: this can be a complex Object.
         */
        public Object val;
        /**
         * The context defines how to retrieve the linked data. Non exhaustive examples:
         * <ul>
         * <li>'processdata': when the linked content data is in the processdata database</li>
         * <li>'ts' when the linked data is retrieved from timeseries database</li>
         * <li>'metadata' when the linked data is retrieved from the metadata database</li>
         * </ul>
         */
    ...
    /**
     * The Header is instantiated as the columns header, or as the rows header.
     * <p/>
     * Header is a collection of header items, separated from the table,
     * content cells. Each item defines a data part as well as a link part. Both optional.
     * <p/>
     * <ul>
     * <li>
     * There is at most one columns header defined for one Table: columns header defines a set
     * of header items, each one targeting its own content column.
     * <li>
     * There is at most one rows header defined for one Table: rows header defines a set
     * of header items, each one targeting its own content row.
     * </li>
     * </ul>
     */
    @JsonIgnoreProperties(ignoreUnknown = true)
    static public class Header {
        ...
        /**
         * The vector of header cells: list of values. There is no pre-defined type. Usually, String is expected in
         * order to define labels.
         */
        public List<Object> data;

        /**
         * default_link contains the default link properties, for the DataLink defined in this.links. See
         * {@link TableContent#default_links}
         */
        public DataLink default_links;
         ...
         * <p>
         * Restriction: it is assumed that the defined objects under theHeader.data are immutable: OK with String,
         * Number, BigDecimal, BigInteger, Boolean (...), otherwise you may have side-effects concerning the copy of
         * theHeader.data.
         *
         * @param theHeader
         */
        ...
        /**
         * Adds new Header item in the table
         *
         * @param data:
         *            header data can be
         *            <ul>
         *            <li>TableElement defining as well data and link part,</li>
         *            <li>or DataLink defining link part and undefined data part,</li>
         *            <li>otherwise the data part with undefined link, demanding an immutable object as data.</li>
         *            </ul>
         * @return this. Convenient for chained calls, for example <code>header.addItem(new
         *         TableElement(data,link)).addItem(link).addItem(1);e</code>.
         * @throws IkatsException
         */
         ...
         *
         * @param data
         * @return this Header. This return enables chained calls, for example: header.addItems(
         *         array1).addItems(array2).
         * @throws IkatsException
         */
         ...
         * @return this: convenient for chained calls, for example header.addItem( item1).addItem( item2).
         * @throws IkatsException
         */
        ...
        /**
         * Gets the this.data: this service is internal to TableManager / Table implementation. You can use getItems(),
         * for external use.
         *
         * @return
         */
         ...
         * @param defaultLink
         *            this link specifies the default link properties, when they are missing in headerLinks items.
         * @param startWithTopLeftCorner
         *            true when provided values are defined from the top-left corner.
         * @return created Header.
         */
        static Header createHeader(List<Object> headerData, List<DataLink> headerLinks, DataLink defaultLink,
                                   boolean startWithTopLeftCorner) {
         ...
         * @throws IkatsException
         *             inconsistency error detected. Ex. when elemH defines a link while this.links == null. Ex. when
        ...
        public void insertItem(int insertedIndexColHeader, TableElement elemH) throws IkatsException {
            try {
                if (elemH.link != null && this.links == null)
                    throw new IkatsException(
                            "Inconsistency: insert header item with link, on a header not managing the links");
                ...
                throw new IkatsException(
                        "Failed to insert " + elemH.toString() + " at header position=" + insertedIndexColHeader, e);
            }
    ...
    /**
     * The header section of the table. The headers are separated from the content.
     */
    @JsonIgnoreProperties(ignoreUnknown = true)
    ...
    /**
     * The description section of the table. Contains different meta information.
     */
    @JsonIgnoreProperties(ignoreUnknown = true)
