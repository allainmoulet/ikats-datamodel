...
public class Table {
	 ...
	 *
	 * @param handledTable
	 *            either from an existing TableInfo, for example, the Table loaded from a json content or from new
	...
	/**
	 * Counts the number of rows in the table. Assumed, but not tested: this Table is consistent - i.e.
	 * this.checkConsistency() does not raise exception -
	 *
	 * @param countColumnsHeaderAsrow
	 *            set to True if the columns header line shall be counted, false otherwise
	 ...
	 * 
	 * Assumed but not tested: this Table is consistent - i.e. this.checkConsistency() does not raise exception -
	 *
	 * @param countRowsHeaderAsColumn
	 *            set to True if the rows header column is counted as a column itself -if managed-
	 ...
	 *
	 * @param theHeader
	 *            Header is the header items container, where is searched the value.
	 * @param value
	 *            Name of the searched header item (item: column or row) to get index
	 ...
	 *
	 * @param columnName
	 *            name of the selected column: this criterion is in the column header.
	 ...
	 * @param castingClass
	 *            the converting class: String.class in order to get toString() on each cell data value (not link);
	 ...
	 * 
	 * @param index
	 *            index of selected row. Note: index is relative to the whole table. If column header exists: 0 points
	 ...
	 * 
	 * @param contentIndex
	 *            index of selected column. Note: index relative to the content section of this Table, i.e
	 *            this.getContent()
	 ...
	 * 
	 * @param contentIndex
	 *            index of selected row. Note: index relative to the content section of this Table, i.e
	 *            this.getContent()
	...
	public void checkConsistency() throws IkatsException {
		...
		internalChecks(sizeColHeaderData, sizeColHeaderLinks, sizeRowHeaderData, sizeRowHeaderLinks,
	...
	private void internalChecks(int sizeColHeaderData, int sizeColHeaderLinks, int sizeRowHeaderData,
		...
		if (nbContentColumnsData == -1)
			throw new IkatsException(MessageFormat.format(TableManager.MSG_INCONSISTENCY_IN_TABLE,
					"CHECK:  nbContentColumnsData != -1", this.toString(), "content columns"));
		...
		if ((nbContentColumnsLinks != -1) && (nbContentColumnsData != nbContentColumnsLinks))
			throw new IkatsException(MessageFormat.format(TableManager.MSG_INCONSISTENCY_IN_TABLE,
					"CHECK:  (nbContentColumnsLinks != -1) => ( nbContentColumnsData == nbContentColumnsLinks)",
					this.toString(), "content columns"));

		// CHECK: nbContentRowsData != -1
		if (nbContentRowsData == -1)
			throw new IkatsException(MessageFormat.format(TableManager.MSG_INCONSISTENCY_IN_TABLE,
					"CHECK:  nbContentRowsData != -1", this.toString(), "content rows"));

		// CHECK: (nbContentRowsLinks != -1) => ( nbContentRowsData ==
		// nbContentRowsLinks)
		if ((nbContentRowsLinks != -1) && (nbContentRowsData != nbContentRowsLinks))
			throw new IkatsException(MessageFormat.format(TableManager.MSG_INCONSISTENCY_IN_TABLE,
					"CHECK:  (nbContentRowsLinks != -1) => ( nbContentRowsData == nbContentRowsLinks)", this.toString(),
					"content rows"));
		...
		if ((sizeColHeaderLinks != -1) && (sizeColHeaderLinks != sizeColHeaderData))
			throw new IkatsException(MessageFormat.format(TableManager.MSG_INCONSISTENCY_IN_TABLE,
					"CHECK:   (sizeColHeaderLinks != -1) => ( sizeColHeaderLinks == sizeColHeaderData)",
					this.toString(), "columns header"));

		// CHECK: (sizeRowHeaderLinks != -1) => ( sizeRowHeaderLinks ==
		// sizeRowHeaderData)
		if ((sizeRowHeaderLinks != -1) && (sizeRowHeaderLinks != sizeRowHeaderData))
			throw new IkatsException(MessageFormat.format(TableManager.MSG_INCONSISTENCY_IN_TABLE,
					"CHECK:   (sizeRowHeaderLinks != -1) => ( sizeRowHeaderLinks == sizeRowHeaderData)",
					this.toString(), "rows header"));

		// Consistency header VS content ...
		// ---------------------------------
		// CHECK: (nbColHeader == 1 && nbRowHeader == 0) => (
		// sizeColHeaderData == nbContentColumnsData)
		if (isHandlingColumnsHeader() && (!isHandlingRowsHeader()) && (sizeColHeaderData != nbContentColumnsData))
			throw new IkatsException(MessageFormat.format(TableManager.MSG_INCONSISTENCY_IN_TABLE,
					"CHECK:   (nbColHeader == 1 && nbRowHeader == 0) => ( sizeColHeaderData == nbContentColumnsData)",
					this.toString(), "header VS content"));

		// CHECK: (nbRowHeader == 1 && nbColHeader == 0) => (
		// sizeRowHeaderData == nbContentRowsData)
		if (isHandlingRowsHeader() && (!isHandlingColumnsHeader()) && (sizeRowHeaderData != nbContentRowsData))
			throw new IkatsException(MessageFormat.format(TableManager.MSG_INCONSISTENCY_IN_TABLE,
					"CHECK:   (nbRowHeader == 1 && nbColHeader == 0) => ( sizeRowHeaderData == nbContentRowsData)",
					this.toString(), "header VS content"));

		// CHECK: (nbRowHeader == 1 && nbColHeader == 1) => (
		// sizeRowHeaderData == nbContentRowsData +1 )
		if (isHandlingRowsHeader() && isHandlingColumnsHeader() && (sizeRowHeaderData != nbContentRowsData + 1))
			throw new IkatsException(MessageFormat.format(TableManager.MSG_INCONSISTENCY_IN_TABLE,
					"CHECK:   (nbRowHeader == 1 && nbColHeader == 1) => ( sizeRowHeaderData == nbContentRowsData +1 )",
					this.toString(), "header VS content"));

		// CHECK: (nbColHeader == 1 && nbRowHeader == 1) => (
		// sizeColHeaderData == nbContentColumnsData + 1)
		if (isHandlingRowsHeader() && isHandlingColumnsHeader() && (sizeColHeaderData != nbContentColumnsData + 1))
			throw new IkatsException(MessageFormat.format(TableManager.MSG_INCONSISTENCY_IN_TABLE,
					"CHECK:   (nbColHeader == 1 && nbRowHeader == 1) => ( sizeColHeaderData == nbContentColumnsData + 1)",
					this.toString(), "header VS content"));
	}
	 ...
	 *            the checked list is supposed to have list items with same size.
	 * @return the size of each items of the checked list. Specific case: returns -1 only when checkedListOfList.size()
	 *         == 0.
	 * @throws IkatsException
	 *             error when one item is null, or at least 2 items have different sizes.
	 */
	private <T> int hasHomogeneousSizes(String checkContext, List<List<T>> checkedListOfList) throws IkatsException {
		...
		for (int itemIndex = 0; itemIndex < checkedListOfList.size(); itemIndex++) {
			...
			if (item == null) {
				String msg = MessageFormat.format(TableManager.MSG_INCONSISTENCY_IN_TABLE,
						"Unexpected null list in " + checkContext, this.toString(), "index=" + itemIndex);
			...
			if (itemIndex == 0) {
				expectedDim = currentSize;
			} else if (!expectedDim.equals(currentSize)) {
				String msg = MessageFormat.format(TableManager.MSG_INCONSISTENCY_IN_TABLE,
						"Defined lists have different sizes in " + checkContext, this.toString(), "index=" + itemIndex);
				throw new IkatsException(msg);
			}
		}
		if (expectedDim == null)
	 ...
	 *            true is initializing a new columns header. false is removing the previous columns header -when needed-
	 * @param withRowsHeader
	 *            true is initializing a new rows header. false is removing the previous rows header -when needed-
	 */
	 ...
	 *            true indicates that the defined headerData and the defined headerLinks begin from the top-left corner
	 * @param defaultLink
	 *            default definition for the links possibly defined by headerLinks. Optional: null if undefined.
	 * @param headerData
	 *            defines the data part of the header. null when no data managed. Note: null is discouraged for data
	 *            part. Pass an empty list if you want to manage data, without specifiing them with this initialization.
	 * @param headerLinks
	 *            defines the links part of the header. null when no links managed. Pass an empty List<DataLink> if you
	 ...
	 * 
	 * @param rowData
	 *            required list of row data values. Accepted types for T: immutable Object (String, Double, Boolean ...)
	 *            or TableElement.
	 ...
	 * 
	 * @param rowHeaderData
	 *            optional row header data. null implies that row header will be ignored, not completed. Accepted types
	 ...
	 * 
	 * @param rowHeaderData
	 *            the inserted header associated to rowdata: required when this.isHandlingRowsHeader()
	 * @param rowData
	 *            required list of row data values. Accepted types for T: immutable Object (String, Double, Boolean ...)
	 *            or TableElement grouping data+link or else DataLink defining one link, without data info.
	 * @return
	 ...
	 * 
	 * @param rowData
	 *            the inserted row. T type is expected to be an immutable Object (simpler case) like String, Integer,
	 *            Double, (...) or TableElement, or else DataLink.
	 *
	 * @throws IkatsException
	 *             inconsistency error, for instance: no rows header managed, or bad value for one parameter, or
	 ...
	 * @param rowData
	 *            list of objects. T type is expected to be TableElement, or DataLink or immutable Object otherwise
	 *            (simple case).
	 * @throws IkatsException
	 ...
	 * 
	 * @param colHeaderData
	 *            the appended header: object typed TableElement, or DataLink or another Object. Simple use String is
	 *            recommended, usually.
	 * @param colData
	 *            the appended column as a List of types T: either Object for data, TableElement for data+link, DataLink
	 ...
	 * 
	 * @param colHeaderData
	 *            the inserted column header data. null is not accepted. (when null: use insertColumn(int, List<T> )
	 *            instead).
	 * 
	 * @param columnData
	 *            the inserted column data. T type is expected to be an immutable Object (simpler case: String, Integer,
	 *            Double, ...) or TableElement, or else DataLink.
	 *
	 * @throws IkatsException
	 *             inconsistency error, for instance: no columns header managed, or bad value for one parameter, or
